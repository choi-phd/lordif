...
function(obj,alpha=0.01,nr=100) {
    ...
    chi12<-matrix(NA,nr,ni)
    chi13<-matrix(NA,nr,ni)
    chi23<-matrix(NA,nr,ni)
    pseudo12.CoxSnell<-matrix(NA,nr,ni)
    pseudo13.CoxSnell<-matrix(NA,nr,ni)
    pseudo23.CoxSnell<-matrix(NA,nr,ni)
    pseudo12.Nagelkerke<-matrix(NA,nr,ni)
    pseudo13.Nagelkerke<-matrix(NA,nr,ni)
    pseudo23.Nagelkerke<-matrix(NA,nr,ni)
    pseudo12.McFadden<-matrix(NA,nr,ni)
    pseudo13.McFadden<-matrix(NA,nr,ni)
    pseudo23.McFadden<-matrix(NA,nr,ni)
    beta12<-matrix(NA,nr,ni)
    ...
    output<-foreach(
        ...
        out<-rundif(1:ni,resp,etheta,group,options$criterion,options$alpha,options$beta.change,options$pseudo.R2,options$R2.change,weights)
        return(out)
    }
